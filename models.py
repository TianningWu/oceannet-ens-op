...
class SpectralConv2d(nn.Module):
    """2D Fourier layer implementing FFT -> linear transform -> inverse FFT"""
    ...
    def forward(self, x):
        ...
        # Apply weights to lower and upper frequency modes
...
class VAE(nn.Module):
    def __init__(self, im_channels, model_config):
        ...
        ##################### Decoder ######################
        self.post_quant_conv = nn.Conv2d(self.z_channels + 256, self.z_channels, kernel_size=1)
    ...
    def encode(self, x):
        ...
        return sample, out, out_for_encoder
    ...
    def forward(self, x):
        # Apply interpolation to achieve the desired size
        x_upsampled = F.interpolate(x, size=(352, 608), mode='bilinear', align_corners=False)
        
        # Perform encoding
        z, encoder_output, out_for_encoder = self.encode(x_upsampled)
        
        # Perform decoding
        out = self.decode(z, out_for_encoder)
        
        # Apply cropping after decoding
        out_cropped = out[:, :, 2:-2, 3:-4]  # Cropping from the sides
        
        return out_cropped, encoder_output
